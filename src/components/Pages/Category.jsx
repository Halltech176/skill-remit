...
const Category = () => {
  ...
  const renderJobs = jobs.map((job, index) => {
    return (
      <motion.div key={index} className="m-4 category_image relative">
        <span>
          <img className="md:w-64 w-32" src={images[index]} alt="images" />
        </span>
        <span className="md:text-xl text-sm text-white absolute top-1/2 left-1/2 -translate-y-1/2 text-center -translate-x-1/2 font-semibold">
          {job}
        </span>
      </motion.div>
    );
  ...
  return (
    <div className="mt-32 mb-12">
      <h1 className="text-center my-10 text-2xl md:text-4xl font-semibold">
